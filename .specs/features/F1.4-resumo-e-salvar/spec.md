...
**Status:** 🚧 IN PROGRESS — P0 + P1 + P2 ✅ (2026-06-01); assíncrona (ack imediato + worker em background). Próximo: **P3** (registro + AGENTS.md + E2E WhatsApp).
...
- **P2 — Refator lib + worker + tool `archive_link` (assíncrona):** ✅ **CONCLUÍDO (2026-06-01).** `extract.ts` self-contained no projeto `knowledge` (cross-import entre tools acoplaria — cada uma tem `node_modules` isolado; F1.2 fica intacta) · `worker.ts` (captura→`summarizeFeynman`→`INSERT`→`omni send`, erro em cada etapa→aviso curto) · `server.ts` expõe `archive_link(url)` que valida e dispara `setsid bun run worker.ts <url>` (desacoplado, `.unref()`) + retorna `{ok,status:"processing"}`. **Validado standalone (2026-06-01):** `omni send` confirmado (probe); worker completo na Wikipedia/Feynman → linha em `knowledge_node` (tópico "Legado de Richard Feynman", 4 pilares) + card entregue no WhatsApp. Callback MVP: instância+número em env (defaults p/ a conta do Lucas).
...
- ~~**Truncamento do texto pro Gemini**~~ → **RESOLVIDO: `MAX_CHARS` 16000 + timeout 6 min.** Como o resumo roda no **worker em background**, priorizamos fidelidade sobre latência (não precisa ser rápido).
