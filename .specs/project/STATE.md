...
**Phase:** **M1 em andamento — F1.2 `fetch_web_content`** (primeira tool real de captura). **Spec escrita 2026-06-01** em `.specs/features/F1.2-fetch-web-content/spec.md` (escolhida como 1ª do M1: continuação direta do harness, independente, testável standalone). Stories: P0 spike DOM parser → P1 tool core → P2 erros → P3 registro+E2E → P4 README.
...
**Action (próximo passo atômico):** **F1.2 / P3 — validação E2E pelo WhatsApp.** P0+P1+P2+P4 ✅ (2026-06-01): tool `tools/fetch-web-content/server.ts` (`fetch_web_content`, parser=`linkedom`, validada standalone em artigo real + casos de erro 404/PDF/inválida), registrada em `.mcp.json` (server `web`) + auto-aprovada (`mcp__web__fetch_web_content`), README escrito. **Falta só:** mandar uma URL real no WhatsApp → conferir `[C] mcp__web__fetch_web_content` no `genie agent log` + resposta curta resumindo (não despejar o texto). ⚠️ Matar órfãos `claude.*linkmind-agent` antes, p/ o novo `.mcp.json` carregar no próximo spawn. Ambiente: WSL2 Ubuntu 26.04, usuário `lucsb` (sudo NOPASSWD) — **sempre `cd ~/linkmind`** (workspace é por-diretório). Engine = `claude` CLI nativo (sonnet) spawnado pelo bridge — auth própria do CLI.
