...
**Feature:** F0.1 → **CONCLUÍDA**. Em andamento: **F0.2 — substituir o stub pela lógica real do agente** (Claude Agent SDK / tools de captura). **Spike de registro de tools CONCLUÍDO 2026-06-01** (ver `.specs/features/F0.2-agente-real-tools/discovery.md`).
**Phase:** F0.2 — spike + **smoke-test CONCLUÍDOS** (2026-06-01). Mecânica validada empíricamente: `genie dir edit <agent> --sdk-mcp-server name:command:args` → persiste como `sdk.mcpServers` no `agent.yaml`; engine `claude` carrega e CHAMA a tool (`mcp__hello__ping` → `pong 🏓`). SDK MCP = `@modelcontextprotocol/sdk` stdio standalone. Detalhes/aprendizados em `discovery.md`.
**Escopo recalibrado (ROADMAP):** F0.2 = **esqueleto + tool harness com 1 tool *dummy***, NÃO captura. DoD = "agente responde ack e chama tool dummy, visível no WhatsApp". A `hello/ping` do smoke-test **é** a dummy da F0.2 (não deletar). Captura real (`fetch_web_content`, `get_youtube_transcript`) é **M1/F1.x**. Spec escrita em `.specs/features/F0.2-agente-real-tools/spec.md`.
**Action (próximo passo atômico):** **P1 ✅ + P2 ✅ CONCLUÍDOS 2026-06-01.** Próximo = **F0.2 / Story P3 (última)** — escrever um README curto em `tools/` documentando o harness: "como adicionar uma tool" (MCP stdio server em `tools/<nome>/server.ts` sob Bun + `@modelcontextprotocol/sdk` + zod → registrar em `agents/linkmind-agent/.mcp.json` → auto-aprovar em `.claude/settings.local.json` via `enableAllProjectMcpServers` + `permissions.allow: ["mcp__<server>__<tool>"]` → verificar via `genie agent log`). Usar `tools/hello-mcp/` como exemplo vivo. Depois disso a F0.2 fecha → segue M1. Ambiente: WSL2 Ubuntu 26.04, usuário `lucsb` (sudo NOPASSWD) — **sempre `cd ~/linkmind`** (workspace é por-diretório). Engine = `claude` CLI nativo (sonnet) spawnado pelo bridge — auth própria do CLI.
